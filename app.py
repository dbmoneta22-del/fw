...
def categorize_ocr(text):
    ...
    for l in lines:
        low = l.lower()
        if any(k in low for k in ["lavora", "dipendente", "azienda", "impiego"]):
            categories["lavoro"].append(l)
        elif any(k in low for k in ["tel", "cell", "telefono", "+39"]):
            categories["telefoni"].append(l)
        elif any(k in low for k in ["immobile", "abitazione", "residente", "via"]):
            categories["immobili"].append(l)
        elif any(k in low for k in ["auto", "veicolo", "targa", "motociclo"]):
            categories["veicoli"].append(l)
        else:
            categories["intervista"].append(l)
